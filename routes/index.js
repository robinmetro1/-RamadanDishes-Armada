...
router.get("/cooktime", async (req, res) => {
  ...
  if (!ingredient || !day) {
    return res
      .status(400)
      .json({
        error: "Both ingredient and day query parameters are required.",
      });
  }
  ...
  if (foundDishes) {
    ...
    ({ maghribMinutes, asrMinutes } =
      await service.getPrayerTimesofRequestedDay(day));

    const cookingTime = service.calculateCookingTime(
      foundDishes,
      maghribMinutes,
      asrMinutes
    );
